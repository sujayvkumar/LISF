...
!BOP
!
! !ROUTINE: readAMSREcnnSnowObs
! \label{readAMSREcnnSnowObs}
!
! !INTERFACE:
subroutine readAMSREcnnSnowObs(source)
!
! !USES:
   use ESMF
   use LVT_histDataMod
   ...
   use AMSREcnnSnow_obsMod,  only : amsrecnnsnowobs
   use map_utils
...
!EOP
   ...
   real                :: snwd_in(amsrecnnsnowobs(source)%nc*amsrecnnsnowobs(source)%nr)
   logical*1           :: lb(amsrecnnsnowobs(source)%nc*amsrecnnsnowobs(source)%nr)
...
! communication with the UA SNOW product generators).
   ...
   if (alarmcheck) then
      ...
      call create_AMSREcnn_filename(amsrecnnsnowobs(source)%odir, &
      ...
      if (file_exists) then
         write(LVT_logunit,*) '[INFO] Reading AMSRE CNN data ',&
                                trim(fname)
         
         allocate(snwd1(amsrecnnsnowobs(source)%nc,   &
              amsrecnnsnowobs(source)%nr))
...
#endif
         do c=1,amsrecnnsnowobs(source)%nc
            do r=1,amsrecnnsnowobs(source)%nr
               if(snwd1(c,r).lt.0.or.snwd1(c,r).gt.1000) then
                  snwd_in(c+(amsrecnnsnowobs(source)%nr-r)*&
                       amsrecnnsnowobs(source)%nc) = -9999.0
               else
                  snwd_in(c+(amsrecnnsnowobs(source)%nr-r)*&
                       amsrecnnsnowobs(source)%nc) = snwd1(c,r)
               endif
            enddo
         enddo
         
!         print*, amsrecnnsnowobs(source)%nc,amsrecnnsnowobs(source)%nr
...
!         stop
         ...
         write(LVT_logunit,*) '[WARN] AMSRE CNN file not found: ',&
              trim(fname)
         snwd_in = -9999.0
      endif
      ...
      do t=1,amsrecnnsnowobs(source)%nc*amsrecnnsnowobs(source)%nr
      ...
      call bilinear_interp(LVT_rc%gridDesc,lb,snwd_in, &
           lo,snwd_out, &
           amsrecnnsnowobs(source)%nc*amsrecnnsnowobs(source)%nr, &
           LVT_rc%lnc*LVT_rc%lnr,  &
           amsrecnnsnowobs(source)%rlat, &
           amsrecnnsnowobs(source)%rlon, &
           amsrecnnsnowobs(source)%w11,  &
           amsrecnnsnowobs(source)%w12,  &
           amsrecnnsnowobs(source)%w21,  &
           amsrecnnsnowobs(source)%w22,  &
           amsrecnnsnowobs(source)%n11,  &
           amsrecnnsnowobs(source)%n12,  &
           amsrecnnsnowobs(source)%n21,  &
           amsrecnnsnowobs(source)%n22,  &
           LVT_rc%udef, iret)
...
end subroutine readAMSREcnnSnowObs

!BOP
!
! !ROUTINE: create_AMSREcnn_filename
! \label(create_AMSREcnn_filename)
!
! !INTERFACE:
subroutine create_AMSREcnn_filename(odir,yr,mo,da,fname)
!
! !USES:
...
!
   ...
   fname = trim(odir)//'/'//trim(fyr)//&
        '/AMSRE_CNN_NH_'//trim(fyr)//'-'//&
        trim(fmo)//'-'//trim(fda)//'.nc'

end subroutine create_AMSREcnn_filename
