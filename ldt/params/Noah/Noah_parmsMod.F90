...
contains
  ...
  subroutine NoahParms_init(flag)
    ...
    if(flag.eq.0) then 
       call NoahParms_init_LIS()
    elseif(flag.eq.1) then 
       call NoahParms_init_LISHydro(flag)
    endif
  end subroutine NoahParms_init

!BOP
! 
! !ROUTINE: NoahParms_init_LIS
! \label{NoahParms_init_LIS}
! 
! !INTERFACE:
  subroutine NoahParms_init_LIS
! !USES:
...
!        call ESMF_ConfigGetAttribute(LDT_config,Noah_struc(n)%pblh%source,rc=rc)
 ...
 end subroutine NoahParms_init_LIS
...
! 
! !INTERFACE:
  subroutine NoahParms_init_LISHydro(flag)
! !USES:
    use LDT_fileIOMod, only : LDT_readDomainConfigSpecs
    use LDT_logMod,    only : LDT_verify
    use LDT_paramOptCheckMod, only: LDT_noahparmsOptChecks, &
...
!        call ESMF_ConfigGetAttribute(LDT_config,Noah_struc(n)%pblh%source,rc=rc)
 ...
 end subroutine NoahParms_init_LISHydro
