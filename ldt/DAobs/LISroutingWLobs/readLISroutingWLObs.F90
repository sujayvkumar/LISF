...
!BOP
! 
! !ROUTINE: readLISroutingWLobs
! \label{readLISroutingWLobs}
!
! !INTERFACE: 
subroutine readLISroutingWLobs(n)
! !USES:
  ...
  use LDT_historyMod
  use LDT_logMod
  use LISroutingWL_obsMod,    only : routingwlobs
!
! !DESCRIPTION: 
...
!EOP
  ...
  character*200    :: fname 
  logical          :: file_exists
  real             :: wl_data(LDT_rc%lnc(n),LDT_rc%lnr(n))
  ...
  integer          :: igrib,nvars
  real             :: wlvalue1d(routingwlobs%nc*routingwlobs%nr)
  real             :: wlvalue2d(routingwlobs%nc, routingwlobs%nr)
  integer          :: c,r
  character*20     :: vname
  integer          :: varid

  interface
     subroutine create_routing_output_filename(n, form, fname, odir, wstyle, wopt, &
     ...
     end subroutine create_routing_output_filename
  end interface
  ...
  call create_routing_output_filename(routingwlobs%nest,               &
                                  routingwlobs%format,             &
                                  fname,                       &
                                  routingwlobs%odir,               &
                                  routingwlobs%wstyle,             &
  ...
  if(file_exists) then 
     write(LDT_logunit,*) '[INFO] reading ROUTING output ',trim(fname)
     if(routingwlobs%format.eq."binary") then 
        ...
        if(file_exists) then 
           ...
           print*, 'ROUTING file ',trim(fname),' does not exist'
           print*, 'Program stopping.. '
           stop
        endif
        call LDT_releaseUnitNumber(ftn)
#endif
     elseif(routingwlobs%format.eq."grib1") then 
        if(routingwlobs%wstyle.ne."WMO convention") then 
        ...
        do 
           ...
           call LDT_verify(iret, &
                'grib_get: indicatorOfParameter failed in readLISroutingWLObs')

           call grib_get(igrib, 'topLevel',topLev, iret)
           call LDT_verify(iret, &
                'grib_get: topLevel failed in readLISroutingWLObs')

           call grib_get(igrib, 'bottomLevel',botLev, iret)
           call LDT_verify(iret, &
                'grib_get: bottomLevel failed in readLISroutingWLObs')

           call grib_get(igrib, 'timeRangeIndicator',trange, iret)
           call LDT_verify(iret, &
                'grib_get: timeRangeIndicator failed in readLISroutingWLObs')

!right now specifically geared for AFWA outputs.            
           if(param_num.eq.201.and.topLev.eq.0.and.botlev.eq.10.and.&
                trange.eq.1) then 

              call grib_get(igrib,'values',wlvalue1d,iret)
              call LDT_verify(iret,&
                   'grib_get: values failed in readLISroutingWLobs')
                           
              call transformRoutingDataToLDTgrid(n,wlvalue1d,wl_data)

           endif

           call grib_release(igrib,iret)
           call LDT_verify(iret, 'error in grib_release in readLISroutingWLObs')
        enddo

        call grib_close_file(ftn)

     elseif(routingwlobs%format.eq."netcdf") then 
#if(defined USE_NETCDF3 || defined USE_NETCDF4) 
...
!  the time averaged data fields will be read in. 
        ...
        iret = nf90_inq_varid(ftn, 'SurfElev_inst', varid)
        vname = 'SurfElev_inst'
        if(iret.ne.0) then 
           vname = 'SurfElev_tavg'
        endif

        if(routingwlobs%datares.eq.LDT_rc%gridDesc(n,10)) then 
           call LDT_readLISSingleNetcdfVar(n,ftn, vname,&
                1,routingwlobs%nc, routingwlobs%nr, wlvalue2d)
        else
           call LDT_readLISSingleNetcdfVar(n,ftn, vname,&
                1,routingwlobs%nc, routingwlobs%nr, wlvalue2d)
        endif
        ...
        do r=1,routingwlobs%nr
           do c=1, routingwlobs%nc
              wlvalue1d(c+(r-1)*routingwlobs%nc) = wlvalue2d(c,r)
           enddo
        enddo
  ...
  call LDT_logSingleDAobs(n,LDT_DAobsData(1)%wl_obs,&
       wl_data,vlevel=1)

#endif
end subroutine readLISroutingWLobs


!BOP
!
! !ROUTINE: create_routing_output_filename
! \label{create_routing_output_filename}
!
! !INTERFACE:
subroutine create_routing_output_filename(n, form, fname, odir, wstyle, wopt, &
                                      run_dd, map_proj, security_class,   &
...
!EOP
   ...
   if(wstyle.eq."4 level hierarchy") then 
      ...
      case default
         call ldt_log_msg('ERR: create_routing_output_filename -- '// &
...
!BOP
! 
! !ROUTINE: transformRoutingDataToLDTgrid
! \label{trasnformDataToLDTgrid}
!
! !INTERFACE: 
 subroutine transformRoutingDataToLDTgrid(n, wl_inp, wl_out)
! !USES:    
   use LDT_coreMod
   use LISroutingWL_obsMod

   implicit none
! !ARGUMENTS: 
   integer         :: n 
   real            :: wl_inp(routingwlobs%nc*routingwlobs%nr)
   real            :: wl_out(LDT_rc%lnc(n),LDT_rc%lnr(n))
!
! !DESCRIPTION: 
...
!EOP
   integer         :: ios
   integer         :: c,r
   logical*1       :: wl_data_b(routingwlobs%nc*routingwlobs%nr)
   real            :: wlobs_ip(LDT_rc%lnc(n)*LDT_rc%lnr(n))
   logical*1       :: wlobs_b_ip(routingwlobs%nc*routingwlobs%nr)

   do r=1,routingwlobs%nr
      do c=1, routingwlobs%nc
         if(wl_inp(c+(r-1)*routingwlobs%nc).ne.LDT_rc%udef) then 
            wl_data_b(c+(r-1)*routingwlobs%nc) = .true. 
         else
            wl_data_b(c+(r-1)*routingwlobs%nc) = .false.
         endif
      enddo
   enddo
...
!-------------------------------------------------------------------------- 
      call bilinear_interp(LDT_rc%gridDesc(n,:),&
           wl_data_b, wl_inp, wlobs_b_ip, wlobs_ip, &
           routingwlobs%nc*routingwlobs%nr, &
           LDT_rc%lnc(n)*LDT_rc%lnr(n), &
           LDT_domain(n)%lat, LDT_domain(n)%lon,&
           routingwlobs%w11, routingwlobs%w12, &
           routingwlobs%w21, routingwlobs%w22, &
           routingwlobs%n11, routingwlobs%n12, &
           routingwlobs%n21, routingwlobs%n22, &
           LDT_rc%udef, ios)
      ...
      call neighbor_interp(LDT_rc%gridDesc(n,:),&
           ...
           LDT_rc%lnc(n)*LDT_rc%lnr(n), &
           LDT_domain(n)%lat, LDT_domain(n)%lon,&
           routingwlobs%n11, LDT_rc%udef, ios)
   else
      call upscaleByAveraging(&
           routingwlobs%nc*routingwlobs%nr,&
           LDT_rc%lnc(n)*LDT_rc%lnr(n),LDT_rc%udef, &
           routingwlobs%n11,wl_data_b, wl_inp, wlobs_b_ip,wlobs_ip)
      
   endif
   
   do r=1,LDT_rc%lnr(n)
      do c=1,LDT_rc%lnc(n)
         if(wlobs_b_ip(c+(r-1)*LDT_rc%lnc(n))) then 
            wl_out(c,r) = wlobs_ip(c+(r-1)*LDT_rc%lnc(n))
         else
            wl_out(c,r) = LDT_rc%udef
         endif
      enddo
   enddo
         
 end subroutine transformRoutingDataToLDTgrid
