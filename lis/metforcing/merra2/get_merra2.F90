...
! startime of 2005-11-02T00:00:00 with time-step of 15mn
  ...
  if( LIS_rc%nts(n).gt.3600 ) then   ! > 1-hr timestep
     write(LIS_logunit,*) '[ERR] When running LIS with MERRA-2, the clock '
  ...
  if ( LIS_get_nstep(LIS_rc,n) == 1 .or. LIS_rc%rstflag(n) == 1 .or. &
     ...
     call LIS_tick(merra2_struc(n)%ringtime,doy1,gmt1,&
     ...
     do r=1,LIS_rc%lnr(n)
        do c=1,LIS_rc%lnc(n)
           if (LIS_domain(n)%gindex(c,r).ne.-1) then
              ...
              if ( order == 1 ) then
                   merra2_struc(n)%metdata1(:,:,LIS_domain(n)%gindex(c,r)) = &
                         merra2_struc(n)%merraforc1(:,:,hr_int1,&  ! Store hour: Current hour (same day)
                         (c+(r-1)*LIS_rc%lnc(n)))
                   merra2_struc(n)%metdata2(:,:,LIS_domain(n)%gindex(c,r)) = &
