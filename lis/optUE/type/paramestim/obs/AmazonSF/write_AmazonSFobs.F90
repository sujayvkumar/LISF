...
!BOP
! !ROUTINE: write_AmazonSFobs
! \label{write_AmazonSFobs}
!
! !REVISION HISTORY:
!  2 Oct 2020: Sujay Kumar; Initial Specification
!
! !INTERFACE: 
subroutine write_AmazonSFobs(Obj_Space)
! !USES: 
  use ESMF
...
! !DESCRIPTION:
!  
!  write the Amazon streamflow data used in parameter estimation to disk
!  
!  The arguments are: 
!  \begin{description}
...
!EOP
  ...
  if(data_update) then 
     ...
     call ESMF_StateGet(Obj_Space,"AmazonSF",sfField,&
          rc=status)
     call LIS_verify(status)
     ...
     if(LIS_masterproc) then 
        ...
        call AmazonSF_obsname('SF',obsname)
...
end subroutine write_AmazonSFobs

!BOP
! 
! !ROUTINE: AmazonSF_obsname
! \label{AmazonSF_obsname}
! 
! !INTERFACE: 
subroutine AmazonSF_obsname(varname, obsname)
! !USES: 
...
!EOP
  ...
  obsname = trim(LIS_rc%odir)//'/PEOBS/'//trim(varname)//'_'//cdate1//'.1gs4r'
  
end subroutine AmazonSF_obsname
