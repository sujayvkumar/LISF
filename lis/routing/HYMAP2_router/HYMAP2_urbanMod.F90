...
contains
  ! ================================================               
  subroutine HYMAP2_calc_urban_drain_stonxt(zdt,zdrvel,zdrtotwth,zdrstomax,zdrout,zdrinf,flddph,zdrsto,zfldsto)
    implicit none
    real, intent(in) :: zdt
    ...
    real, intent(in) :: flddph
    real, intent(out) :: zfldsto
    ...
    if(zdrsto>zdrstomax)then
      ...
      zfldsto=zfldsto+max(0.,zdrsto-zdrstomax)
      zdrsto=zdrstomax
    else
      !water intake through gutters
      zdrcapnow=max(0.,zdrstomax-zdrsto)
      zdrflwpot=min(zfldsto,flddph*zdrvel*zdrtotwth*zdt)
      zsrfstoin=min(zdrcapnow,zdrflwpot)
      zfldsto=zfldsto-zsrfstoin
  ...
  subroutine HYMAP2_gen_urban_drain_maps(nseqall,drrad,drlgh,drden,drwth,drblk,&
                        grarea,next,flowmap,droutlet,drstomax,drtotwth,drnoutlet,drtotlgh)
    implicit none
    ...
    integer, intent(out)  :: droutlet(nseqall) !urban drainage outlet flag: 0 - drainage network; 1 - river
