...
! === urban drainage and flood modeling ===
  ...
  integer, allocatable :: droutlet(:)       !outlet flag: 0 - drainage network; 1 - river
  integer, allocatable :: droutlet_glb(:)       !global outlet flag: 0 - drainage network; 1 - river
   ...
   !ag(27Apr2020)
    ...
    do n=1, LIS_rc%nnest
      ...
      if(HYMAP2_routing_struc(n)%insertflag==1)then
        ...
        call ESMF_ConfigFindLabel(LIS_config,&
             "HYMAP2 discharge direct insertion time series size:",rc=status)
        call ESMF_ConfigGetAttribute(LIS_config,&
             HYMAP2_routing_struc(n)%ntinsert,rc=status)
        call LIS_verify(status,&
             "HYMAP2 discharge direct insertion time series size: not defined")
...
!TBD: SVK - block below needs update for parallelism
    ...
    do n=1, LIS_rc%nnest
      if(HYMAP2_routing_struc(n)%flowtype==4)then
        ...
        call HYMAP2_gather_tiles_int(n,HYMAP2_routing_struc(n)%droutlet,HYMAP2_routing_struc(n)%droutlet_glb) 
      endif
    enddo

    !ag(8Aug2020) - read dischaarge data for direct insertion
    do n=1, LIS_rc%nnest
!TBD: SVK - block below needs update for parallelism
       if(HYMAP2_routing_struc(n)%insertflag==1)then
