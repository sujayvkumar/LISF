...
subroutine HYMAP2_model_core(n,it,mis,nseqall,nz,time,dt,  &
     ...
     flowtype,drvel,drtotwth,drrad,drman,drslp,drtotlgh,drnoutlet,droutlet,drstomax,drout,drsto)
  ! ================================================
  !HyMAP core 
  !Augusto Getirana 
  !6th Dec 2010
  ...
  real,    intent(in)  :: drrad(nseqall)    
  real,    intent(in)  :: drman(nseqall)   
  real,    intent(in)  :: drslp(nseqall)   
  ...
  real,    intent(in)  :: droutlet(nseqall)    !urban drainage outlet id [-]: 0 - network; 1 outlet
  ...
  real, allocatable    :: drout_glb(:)  !global urban drainage inflow  [m3/s]
...
!print*,ic,ic_down,nseqall,size(flddph1_glb),flddph1_glb(ic_down)
    ...
    !urban drainage global variables
    if(flowtype==4)then
      if(droutlet(ic)==0)then
        ic_down=next(ic)
        drsto_down=drsto_glb(ic_down)
      elseif(droutlet(ic)==1)then
        drsto_down=drsto(ic)
      else
        write(LIS_logunit,*)"[ERR] Wrong urban drainage outlet id"
        call LIS_endrun()
      endif
    endif
    ...
    if(flowmap(ic)==1)then
      ...
      elseif(floodflag==0)then
        ...
        write(LIS_logunit,*)"[ERR] HYMAP floodplain dynamics: unknown value"
        call LIS_endrun()
      endif
    !ag (27Apr2020)
...
! ================================================
  ...
  if(LIS_masterproc) then 
    ...
    do ic=1,LIS_rc%glbnroutinggrid(n)
      if(HYMAP2_routing_struc(n)%outlet_glb(ic)==0)then
        ...
        rivinf_glb(ic_down)=rivinf_glb(ic_down)+rivout_glb(ic)
        fldinf_glb(ic_down)=fldinf_glb(ic_down)+fldout_glb(ic)
      endif
    enddo
  endif
  ...
  if(flowtype==4)then
    ...
    if(LIS_masterproc) then 
      ...
      do ic=1,LIS_rc%glbnroutinggrid(n)
        ic_down=HYMAP2_routing_struc(n)%next_glb(ic)
        if(HYMAP2_routing_struc(n)%droutlet_glb(ic)==0)then
          drinf_glb(ic_down)=drinf_glb(ic_down)+drout_glb(ic)
        else
          rivinf_glb(ic_down)=rivinf_glb(ic_down)+drout_glb(ic)
        endif          
      enddo
    endif
    ...
    do ic=1,nseqall 
      call HYMAP2_calc_urban_drain_stonxt(dt,drvel,drtotwth(ic),drstomax(ic),drout(ic),drinf(ic),flddph1(ic),drsto(ic),fldsto(ic))
    enddo
  endif
  ...
  if(resopflag==1)then
    do ic=1,nseqall 
      ...
      if(minval(abs(HYMAP2_routing_struc(n)%resopaltloc-icg))==0)then
        ...
        ic_down=HYMAP2_routing_struc(n)%next_glb(icg)
        !inflow=sum(rivout_glb+fldout_glb,next_glb==icg)
        inflow=rivinf_glb(ic)+fldinf_glb(ic)
        rivout(ic)=rivout(ic)+fldout(ic)
        rivinf_glb(ic_down)=rivinf_glb(ic_down)+fldinf_glb(ic_down)
        fldout(ic)=0.
        fldinf_glb(ic_down)=0.
        call HYMAP2_resop_main(mis,nseqall,nz,time,dt,next(ic),rivsto(ic),fldsto(ic),runoff(ic),&
             rivout(ic),rivinf_glb(ic_down),rivvel(ic),fldvel(ic),&
...
!#if 0
!print*,HYMAP2_routing_struc(n)%insertloc
  ! ================================================
  !ag (9Aug2020)
...
!print*,ic,icg
      if(minval(abs(HYMAP2_routing_struc(n)%insertloc-icg))==0)then
        ...
        fldinf_glb(ic_down)=0.
!print*
!print*,rivout(ic),rivinf_glb(ic_down)
        !get linear intepolatee observed streamflow
        ...
        rivout(ic)=disobs
        rivinf_glb(ic_down)=rivinf_glb(ic_down)-disdif
!print*,time,rivout(ic),rivinf_glb(ic_down)
      endif
    enddo
  endif
!#endif
